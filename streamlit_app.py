...
api_key = "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"
...
#     tab1, tab2, tab3, tab4 = st.tabs(["Data Viewer", "Analytics", "Grant Themes", "Refresh Data Source"])
...
#                     chat_responses.append(f"Opportunity Number {opportunity_number}:\n{response}")
#                 st.session_state['chatgpt_responses_data_viewer'] = chat_responses
#             st.text_area("ChatGPT Responses (Data Viewer):", value="\n\n".join(st.session_state['chatgpt_responses_data_viewer']), height=400, key="data_viewer_responses")
...
#             display_data_insights(st.session_state['filtered_df'])
...
#                 st.text_area("ChatGPT Responses (Grant Themes):", value="\n\n".join(st.session_state['chatgpt_responses_grant_themes']), height=400, key="grant_themes_responses")
...
#                         grant_url = f"https://grants.gov/search-results-detail/{opportunity_id}"
#                         prompt = f"Review the following grant description related to '{search_terms}'. Confirm if it's relevant by responding with 'Yes' or 'No' or 'Relevant', and provide a concise explanation. Highlight key eligibility criteria for US grants. Opportunity ID: {opportunity_number}, Title: '{opportunity_title}':\n\n{description}"
#                         response = call_chat_gpt(prompt)
#                         if "Yes" in response:
#                             # chat_responses.append(f"<strong>Opportunity Number {opportunity_number}:</strong> {response}<br><a href='{grant_url}' target='_blank'>View Grant Details</a><br><br>")
#                             chat_responses.append(f"<strong>Opportunity Number {opportunity_number}:</strong> {response}<br><a href='{grant_url}' target='_blank'>View Grant Details</a><br><br>")
#                         # chat_responses.append(f"Opportunity Number {opportunity_number}:\n{response}")
#                     st.session_state['chatgpt_responses_data_viewer'] = chat_responses
#             st.text_area("ChatGPT Responses (Data Viewer):", value="\n\n".join(st.session_state['chatgpt_responses_data_viewer']), height=400, key="data_viewer_responses")
